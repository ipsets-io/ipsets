...
func (p *Provider) Meta() provider.Meta {
	return provider.Meta{
		...
		Sets: []provider.Set{
			...
			service("ebs", "EBS", "", "EBS"),
			service("efs", "EFS", "", "EFS"),
			service("dynamodb", "DynamoDB", "", "DYNAMODB"),
			service("aurora-dsql", "Aurora DSQL", "", "AURORA_DSQL"),
			...
			service("workspaces-gateways", "WorkSpaces gateways", "", "WORKSPACES_GATEWAYS"),
			service("connect", "Amazon Connect", "", "AMAZON_CONNECT"),
			service("chime-voiceconnector", "Chime Voice Connector", "", "CHIME_VOICECONNECTOR"),
			service("chime-meetings", "Chime Meetings", "", "CHIME_MEETINGS"),
			service("kinesis-video-streams", "Kinesis Video Streams", "", "KINESIS_VIDEO_STREAMS"),
			service("media-package-v2", "MediaPackage v2", "", "MEDIA_PACKAGE_V2"),
			service("ivs-realtime", "IVS Real-Time", "", "IVS_REALTIME"),
			service("ivs-low-latency", "IVS Low-Latency", "", "IVS_LOW_LATENCY"),
		},
	}
}
